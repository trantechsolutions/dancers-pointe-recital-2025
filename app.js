...
const authorizedUsers = ["jonny5v@gmail.com", "bdwill@gmail.com"];
...
function App() {
    ...
    const updateCurrentActNumber = async (newNumber) => {
        ...
        const docRef = doc(db, `artifacts/${appId}/public/data/show_status`, selectedShow);
        try {
            await setDoc(docRef, { currentActNumber: newNumber });
        } catch (err) {
    ...
    const searchResults = useMemo(() => {
        ...
        showData.acts.forEach(act => {
            if ((act.performers || []).some(p => p.toLowerCase().includes(q))) {
                if (!matching.has(act.number)) matching.set(act.number, act);
            }
        });
